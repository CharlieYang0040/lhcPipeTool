...
class ProjectVersionService:
    def __init__(self, connector):
        ...
        self.version_model = ProjectVersion(connector)
        self.worker_model = Worker(connector)

    def create_version(self, project_id, version_number=None, worker_name=None, 
                      file_path=None, preview_path=None, comment=None, status=None):
        """새 프로젝트 버전 생성"""
        try:
            self.logger.info(f"프로젝트 버전 생성 시작 - project_id: {project_id}")
            ...
            # 새 버전 생성
            self.logger.info(f"새 프로젝트 버전 생성 시도 - version_name: {version_name}, version_number: {version_number}")
            ...
            self.logger.error(f"프로젝트 버전 생성 중 예외 발생: {str(e)}", exc_info=True)
            return False
    ...
    def _get_next_version_number(self, project_id):
        """다음 프로젝트 버전 번호 조회"""
        try:
            self.logger.debug(f"다음 프로젝트 버전 번호 조회 시작 - project_id: {project_id}")
            versions = self.version_model.get_all_versions(project_id)
            self.logger.debug(f"기존 프로젝트 버전 목록: {versions}")
            
            if not versions:
                self.logger.debug("기존 프로젝트 버전 없음, 버전 1 반환")
                return 1
            ...
            self.logger.debug(f"다음 프로젝트 버전 번호: {next_version}")
            return next_version
            
        except Exception as e:
            self.logger.error(f"다음 프로젝트 버전 번호 조회 실패: {str(e)}")
            return 1
    ...
    def get_shot_info(self, project_id):
        """샷 정보 조회"""
        try:
            cursor = self.connector.cursor()
            query = """
                SELECT shots.*, 
                       (SELECT version_number 
                        FROM versions 
                        WHERE versions.project_id = shots.id 
    ...
    def get_version(self, project_id, version_number):
        """특정 샷의 특정 버전 조회"""
        try:
            cursor = self.connector.cursor()
            cursor.execute("""
            ...
            self.logger.error(f"버전 조회 실패: project_id={project_id}, version_number={version_number}, error={str(e)}")
            return None
    ...
    def get_latest_version(self, project_id):
        """프로젝트의 최신 버전 조회"""
        return self.version_model.get_latest_version(project_id)
